...
func GetRecommendDriverTrips(user models.User) (trips []models.ResponseTrip, err error) {
	query := "SELECT *,ROUND(6378.138 * 2 * ASIN(SQRT(POW(SIN((? * PI() / 180 - from_lat * PI() / 180) / 2),2) + COS(? * PI() / 180) * COS(from_lat * PI() / 180) * POW(SIN((? * PI() / 180 - from_lon * PI() / 180) / 2), 2))) * 1000) AS distance FROM driver_trip where travel_time>=unix_timestamp(now()) ORDER BY distance ASC limit 20"
