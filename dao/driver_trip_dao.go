...
func GetRealTimeDriverTrip() (trips []models.ResponseTrip, err error) {
	query := "select * from (select * from driver_trip order by create_time desc) as a group by phone limit 20"
...
func SaveDriverTrip(trip models.DriverTrip) (models.DriverTrip, error) {
	...
	_, err = cacheDB.NamedExec("INSERT INTO driver_trip "+
		...
		",`from_region`,`from_city`,`from_accurate_address`,`from_vague_address`,`destination_region`,`destination_city`,`destination_accurate_address`,`destination_vague_address`,`source`,`mileage`,`seat_num`,`complete`,`msg`,`from_district`,`destination_district`) VALUES "+
		"(:guid,:username,:nickname,:phone,:create_time, :travel_time,  :travel_time_title, :from_lon, :from_lat, :pay_price, :surplus,  :destination_lon, :destination_lat"+
		",:from_region,:from_city,:from_accurate_address,:from_vague_address,:destination_region,:destination_city,:destination_accurate_address,:destination_vague_address,:source,:mileage,:seat_num,:complete,:msg,:from_district,:destination_district)", &trip)
	if err == nil {
